...
import { FaBars, FaUsers, FaCalendarCheck, FaCog, FaSignOutAlt, FaImages, FaClipboardList, FaHospital } from "react-icons/fa";
...
const AdminDashboard = () => {
  ...
  console.log("AdminDashboard is rendered"); 

  return (
    <div className="flex h-screen bg-gray-100">
      {/* Sidebar */}
      <div className={` ${isOpen ? "w-64" : "w-16"} bg-blue-900 h-screen p-5 pt-8 duration-300 fixed`}>
        <FaBars
        ...
        <div className="flex flex-col gap-6 mt-10">
          ...
          <button onClick={handleLogout} className="flex items-center gap-4 p-2 text-white hover:bg-red-600 rounded-md cursor-pointer">
      ...
      <div className="flex-1 ml-16 md:ml-64">
        ...
        <div className="flex justify-between items-center bg-white p-4 shadow-md">
          <h1 className="text-2xl font-bold text-gray-700">Mechi Eye&Aesthitic</h1>
        ...
        {/* Content Area */}
        <div className="p-6">
          <Outlet /> {/* This will load different pages inside the Admin Panel */}
        </div>
      </div>
    </div>
  );
...
const SidebarItem = ({ icon, title, isOpen, path }) => (
  <Link to={path} className="flex items-center gap-4 p-2 hover:bg-blue-800 text-white rounded-md cursor-pointer">
